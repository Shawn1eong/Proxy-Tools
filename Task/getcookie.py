...
from .. import jdbot, chat_id, img_file, _JdDir
...
def autoReplaceCookie(filePath,jd_cookie):
    newlines = ''
    newConfig = ''
    pt_pin = jd_cookie.split('pt_pin=')[1]
    num = len(open(filePath,'r',encoding='utf-8').readlines())  #行数
    f = open(filePath,'r',encoding='utf-8')
    for i in range(num+1):
        line = f.readline()
        if pt_pin in line :
            oldCookie = line.split('"')[1] 
            line = line.replace(oldCookie,jd_cookie)
            newConfig = True
            print(f'新Cookie已自动替换\n{line}')
        newlines += line
    if newConfig:
        f = open(filePath,'w',encoding='utf-8')
        f.write(newlines)
        f.close()
        # await @jbot.send_message('已自动替换Cookie')
    else :
        print('新账号Cookie，需手动增加')
    return newConfig
    ...
    try:
        ...
        while login:
            ...
            if data.get("errcode") == 0:
                ...
                newConfig = autoReplaceCookie(f'{_JdDir}/config/config.sh',jd_cookie)   #调用自动替换cookie函数
                if newConfig :
                    await jdbot.send_message(chat_id, '已自动替换Cookie')
                else :
                    await jdbot.send_message(chat_id, '新账号Cookie无法替换，需手动增加')
                return
